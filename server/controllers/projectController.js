 ...
 * {
 *   name: "Project Name",
 *   students: [ { regNo, name, emailId, reviews, deadline }, ... ],
...
export async function createProject(req, res, next) {
  ...
  try {
    ...
    const schemaInternshipType = markingSchema.internshipType || 'none';
    ...
    const studentIds = await Promise.all(
      studentDetails.map(async (studentObj) => {
        ...
        const student = new Student({
          ...
          internshipType: studentInternshipType,
        });
    ...
    return res.status(201).json({
      ...
      data: {
        ...
        internshipType: schemaInternshipType,
      },
    });
  } catch (error) {
    await session.abortTransaction();
    ...
    if (
      ...
      error.message.includes("Invalid internship type") ||
      error.message.includes("internshipType can only have values")
    ) {
      return res.status(400).json({
        success: false,
  ...
  try {
    ...
    const projects = await Project.find(filter)
      .populate({
        path: "students",
        model: "Student",
        select: "regNo name emailId reviews deadline school department PAT requiresContribution contributionType internshipType",
      })
      .populate({
        path: "guideFaculty",
        model: "Faculty",
    ...
    const processedProjects = projects.map((project) => {
      const processedStudents = project.students.map((student) => {
        ...
        return {
          ...
          internshipType: student.internshipType || 'none',
        };
      });
...
export async function getAllGuideProjects(req, res) {
  try {
    ...
    console.log("✅ All projects processed with full population and data");
    ...
    projectsWithSchemas.forEach((project) => {
      ...
      project.students.forEach((student) => {
        console.log(
          `  - ${student.name}: ${Object.keys(student.reviews || {}).length} reviews, requiresContribution: ${student.requiresContribution}, contributionType: ${student.contributionType}, internshipType: ${student.internshipType}`
        );
      });
      console.log(`📋 Schema: ${project.markingSchema ? "YES" : "NO"}`);
      if (project.markingSchema) {
        console.log(
          `📋 Schema Reviews: ${project.markingSchema.reviews?.length || 0} total`
        );
        console.log(
  ...
  try {
    ...
    const processedProjects = panelProjects.map((project) => {
      ...
      const processedStudents = project.students.map((student) => {
        ...
        console.log(`📝 Panel student ${student.name} internshipType:`, student.internshipType);
    ...
    console.log("✅ All panel projects processed with full population and data");
    ...
    projectsWithSchemas.forEach((project) => {
      ...
      if (project.markingSchema) {
        console.log(
          `📋 Panel Schema Reviews: ${project.markingSchema.reviews?.length || 0} total`
        );
        console.log(
      ...
      for (const [reviewType, reviewData] of Object.entries(
        ...
        const newReviewData = {
          ...
          ...(reviewData.contribution !== undefined && {
            contribution: String(reviewData.contribution),
          }),
        };
  ...
  try {
    ...
    const requiredProject = await Project.findOne({ _id: projectId })
      .populate({
        path: "students",
        model: "Student",
        select: "regNo name emailId reviews deadline school department requiresContribution contributionType internshipType",
      })
      .populate({
        path: "guideFaculty",
        model: "Faculty",
...
export async function createProjectsBulk(req, res) {
  ...
  try {
    ...
    console.log("📝 [SCHEMA INFO] internshipType:", schemaInternshipType);
