...
// ✅ SIMPLIFIED: Normalize student data without PPT logic
function normalizeStudentData(student) {
  ...
  Object.keys(reviews).forEach(reviewKey => {
    ...
    if (review && typeof review === 'object') {
      ...
      if (review.contribution !== undefined) {
        review.contribution = review.contribution;
      }

      // ✅ Add internship type if present
      if (review.internshipType !== undefined) {
        review.internshipType = review.internshipType;
      }
    }
  });
...
const PanelContent = React.memo(({ 
  ...
  console.log('🔄 [PanelContent] Rendering with refreshKey:', refreshKey);

  const teamsWithReviewData = useMemo(() => {
    return teams.map(team => {
      // Get panel reviews only
      let reviewTypes = [];
    ...
    <div className="bg-white shadow-xl rounded-2xl overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {teamsWithReviewData.map(team => {
          const { reviewTypes, deadlines } = team;
              ...
              <div key={team.id} className="bg-yellow-50 border-l-4 border-yellow-400 p-6 m-4 sm:m-6 rounded-r-xl">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-yellow-700 mt-1">No panel reviews configured for this project</p>
                  </div>
                </div>
              </div>
            );
                    ...
                    {reviewTypes.map(reviewType => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-3 text-white text-sm font-medium rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 ${
                            isPassed ? 'opacity-75' : ''
...
const Panel = () => {
      ...
      if (projectsRes.data?.success) {
        ...
        mappedTeams = projects.map(project => {
          ...
          return {
            ...
            panel: project.panel,
            bestProject: project.bestProject || false
          };
        });
  ...
  // ✅ SIMPLIFIED: Remove PPT logic from submission
  const handleReviewSubmit = useCallback(async (teamId, reviewType, reviewData, pptObj, patUpdates) => {
    try {
      ...
      const studentUpdates = team.students.map(student => {
        ...
        // ✅ Add internship type at student level
